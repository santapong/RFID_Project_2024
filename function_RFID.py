...
# คลาสสำหรับการทำงานของ RFID
class Function_RFID:
    def __init__(self):
        # ตั้งค่าโหมด GPIO
        GPIO.setmode(GPIO.BCM)
        self.IR_PIN_1 = 14  # กำหนดพินสำหรับเซ็นเซอร์ IR
        GPIO.setup(self.IR_PIN_1, GPIO.IN)
        ...
        self.vendor_id = 0x076b  # กำหนด Vendor ID ของอุปกรณ์
        self.product_id = 0x5128  # กำหนด Product ID ของอุปกรณ์
        self.serial_number = "01010053505638313013421947195730"  # กำหนด Serial Number ของอุปกรณ์
    ...
    def sensor(self):
        while self.running:
            self.input_sensor = GPIO.input(self.IR_PIN_1)  # อ่านค่าจากเซ็นเซอร์
        ...
        if not self.is_loaded:
            ...
            if self.timer_running and (time.time() - self.start_time >= 0.1):
                if len(self.tag_id) > 5:  # ตรวจสอบว่ามีข้อมูล tag_id ไหม
                    ...
                    self.rfid_data['loaded_tag_id'] = ''.join(self.tag_id)  # บันทึกข้อมูล tag_id
                    self.rfid_data['loaded_timestamp'] = (timestamp_date, timestamp_time)  # บันทึกเวลาที่โหลด
                    print(f"Loaded Tag ID: {self.rfid_data['loaded_tag_id']}, Date: {timestamp_date}, Time: {timestamp_time}")
                    self.update_load_history(self.rfid_data['loaded_tag_id'], timestamp_date, timestamp_time)  # อัปเดตประวัติการโหลด
                    self.timer_running = False  # หยุด Timer
                    self.is_loaded = True  # ตั้งค่าสถานะว่าโหลดแล้ว
                else:
                    self.rfid_data['unloaded_tag_id'] = ''
                    self.rfid_data['unloaded_timestamp'] = ('', '')  # ถ้าไม่มีข้อมูล ให้ล้างข้อมูล
                    self.tag_id.clear()
    ...
    def unload(self):
        if self.is_loaded :
            timestamp_date, timestamp_time = self.timestamp()  # ดึงเวลาปัจจุบัน
            self.rfid_data['unloaded_tag_id'] = ''.join(self.tag_id)  # บันทึกข้อมูล tag_id ที่ปลด
            self.rfid_data['unloaded_timestamp'] = (timestamp_date, timestamp_time)  # บันทึกเวลาที่ปลด
            print(f"Unloaded Tag ID: {self.rfid_data['unloaded_tag_id']}, Date: {timestamp_date}, Time: {timestamp_time}")
            self.update_unload_history(self.rfid_data['unloaded_tag_id'], timestamp_date, timestamp_time)  # อัปเดตประวัติการปลด
            self.is_loaded = False  # ตั้งค่าสถานะว่ายังไม่ได้โหลด
            if self.condition:
                self.rfid_data['loaded_tag_id'] = ''
                self.rfid_data['loaded_timestamp'] = ('', '')  # ล้างข้อมูลที่โหลด
                self.tag_id.clear()
...
if __name__ == "__main__":
    rfid_project = Function_RFID()  # สร้างอ็อบเจกต์จากคลาส
    try:
        while True:
            time.sleep(1)  # หน่วงเวลา 1 วินาที
    except KeyboardInterrupt:
        rfid_project.running = False  # หยุดการทำงานเมื่อกด Ctrl+C
        rfid_project.thread.join()  # รอให้ Thread สิ้นสุด
